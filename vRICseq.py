...
def vRICpipeline(refseq,fdata,sdata,title):
    ...
    os.system("perl /storx/ccl/workapce/vRICseq/TGEV/creat_gapped_and_Align_pair_bed.VirusOnlyVersion.pl ./z3.update_alignPair/update.mergeBwa.interaction.sam")
    #注意修改perl脚本中的id号
    ...
    os.system("perl ../../sam_to_loops_split.pl %s_interaction.Chimeric.HQ.sam NC_038861.1 0 40000 > %s_interaction.loops"%(title,title))#注意修改基因组名称
    command4='''awk -F '\t' 'BEGIN{OFS="\t"}{if(($3<=28586)&&($6<=28586)) print $0}' %s_interaction.loops > %s_interaction.filt.loops'''%(title,title)
    ...
    os.system("java -jar /storx/max/workspace/vRICseq/D.visual_by_JuiceBox/scripts/common/juicer_tools.jar pre -r 1,2,5,10,25,50 %s_in_Virion.sort.list %s_in_Virion.hic /storx/ccl/workapce/vRICseq/TGEV/TGEV.chrom.sizes"%(title,title))
